...
export class MediumProvider implements ContentProvider {
    ...
    try {
      ...
      try {
        const rssRes = await fetch(rssUrl, {
          headers: {
            "User-Agent":
              "Mozilla/5.0 (compatible; MyTube-Learning-Bot/1.0)",
          },
  ...
  private parseRSS(xml: string, limit: number): Content[] {
    const items: Content[] = [];
  ...
  private extractImageFromContent(block: string): string {
  ...
  private async searchViaDDG(query: string, limit: number): Promise<Content[]> {
    ...
    try {
      ...
      for (let i = 1; i < parts.length && results.length < limit; i++) {
        ...
        if (!actualUrl.includes("medium.com")) continue;
