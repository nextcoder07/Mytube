  ...
  private static interleaveBySource(
    ...
    while (added < reserved) {
      ...
      for (let i = 0; i < providerQueues.length && added < reserved; i++) {
        ...
        if (alreadyFromSource >= minPerSource) continue;
